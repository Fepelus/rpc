...
class SandpitState implements State {
  ...
  SandpitState.from(Stack stack, String sandpit) {
  ...
  @override State enter() => new StackState.from(_model.stack.push(_model.sandpit));
  ...
  @override State delete() =>
    _model.sandpit.length > 1
      ...
      : new StackState.from(_model.stack);
...
class StackState implements State {
  ...
  @override State delete() =>
    _model.stack.isEmpty
      ...
      : new StackState.from(_model.stack.shrink);

  @override State decimalPoint() => new SandpitState.from(_model.stack, "0.");
  @override State add() {
    var addendPop = _model.stack.pop();
    var augendPop = addendPop.stack.pop();
    return new StackState.from(augendPop.stack.push((augendPop.value + addendPop.value).toString()));
  }
  @override State subtract() {
    var subtrahendPop = _model.stack.pop();
    var minuendPop = subtrahendPop.stack.pop();
    return new StackState.from(minuendPop.stack.push((minuendPop.value - subtrahendPop.value).toString()));
  }
  @override State multiply() {
    var multiplierPop = _model.stack.pop();
    var multiplicandPop = multiplierPop.stack.pop();
    return new StackState.from(multiplicandPop.stack.push((multiplicandPop.value * multiplierPop.value).toString()));
  }
  @override State divide() {
    var divisorPop = _model.stack.pop();
    var dividendPop = divisorPop.stack.pop();
    return new StackState.from(dividendPop.stack.push((dividendPop.value / divisorPop.value).toString()));
  }
  @override State power() {
    var exponentPop = _model.stack.pop();
    var nPop = exponentPop.stack.pop();
    return new StackState.from(nPop.stack.push(pow(nPop.value, exponentPop.value).toString()));
  }
  @override State plusMinus() {
    PoppedTuple pop = _model.stack.pop();
    return new StackState.from(pop.stack.push((pop.value * -1).toString()));
  }
  @override State swap() {
    var originalHeadPop = _model.stack.pop();
    var originalDeputyPop = originalHeadPop.stack.pop();

    return new StackState.from(
